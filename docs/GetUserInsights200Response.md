...
**SeatsCapped** | Pointer to **bool** | True only on tiers with a hard seat cap (Free, Founder). On unlimited/per-seat tiers seats_alloc merely tracks the live mailbox count, so seats_used &#x3D;&#x3D; seats_alloc is normal and must not be read as &#39;at capacity&#39;. | [optional] 
...
GetSeatsCapped returns the SeatsCapped field if non-nil, zero value otherwise.

### GetSeatsCappedOk

`func (o *GetUserInsights200Response) GetSeatsCappedOk() (*bool, bool)`

GetSeatsCappedOk returns a tuple with the SeatsCapped field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.
...
SetSeatsCapped sets SeatsCapped field to given value.
